...
namespace SeedFinding.Services
{
    class DynamoDB
    {
        ...
        {
            _tableName = (await ssmClient.GetParameterAsync(new GetParameterRequest { Name = "/seed-job-ddb/table-name" })).Parameter.Value;
        ...
        public async Task<DescribeTableResponse> DescribeTableAsync()
        {
            ...
            return await _client.DescribeTableAsync(new DescribeTableRequest { TableName = _tableName });
        }
        ...
        public async Task UpdateJob(Job job)
        {
            await _client.UpdateItemAsync(new UpdateItemRequest
            {
                ...
                {
                    {"user_id", new AttributeValue {S = job.UserId}},
                    {"job_id", new AttributeValue {S = job.JobId}}
                },
